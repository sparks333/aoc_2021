...
def est_cost(game_map):
    ...
    for y in range(1, 6):
        for x in range(1, 12):
            if game_map[y,x] > 0:
                if (2*game_map[y,x])+1 != x:
                    total_cost += (10**(game_map[y,x]-1)) * (abs(x - ((2*game_map[y,x])+1)) + y)
                elif y == 2 and (2*game_map[3, x])+1 != x:
                    total_cost += (10**(game_map[y,x]-1))*4
...
def valid_move(y_start, x_start, x_end, y_end, game_map):
    ...
    if y_end > 1 and np.any(game_map[y_end:5, x_end] != game_map[y_start, x_start]):
        return False

    # Moving finalized pieces (A)
    if game_map[y_start, x_start] == 1 and x_start == 3:
        if y_start > 2 and np.all(game_map[y_start:6, x_start]
            return False
...
def move(y_start, x_start, x_end, y_end, game_map):
